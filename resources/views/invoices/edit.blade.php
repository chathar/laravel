...
@section('actions')
    <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 transition ease-in-out duration-150">
        Cancel
    </a>
@endsection
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="invoice_number" id="invoice_number" required value="{{ old('invoice_number', $invoice->invoice_number) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="client_id" id="client_id" required
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ $invoice->client_id == $client->id ? 'selected' : '' }}>{{ $client->name }} ({{ $client->company }})</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button type="button" onclick="addItem()" class="text-sm font-bold text-blue-600 hover:text-blue-800 flex items-center transition-colors">
                    ...
                    <div id="items-container" class="space-y-4">
                        ...
                        <div class="item-row bg-gray-50 rounded-xl p-6 border border-gray-100 relative group">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                <div class="md:col-span-6">
                                    ...
                                    <input type="text" name="items[{{ $index }}][description]" required value="{{ $item->description }}"
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="number" name="items[{{ $index }}][quantity]" required step="1" value="{{ $item->quantity }}" onchange="calculateTotals()"
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="number" name="items[{{ $index }}][unit_price]" required step="0.01" value="{{ $item->unit_price }}" onchange="calculateTotals()"
                                ...
                                <div class="md:col-span-2 flex flex-col justify-end">
                                    ...
                                    <div class="text-sm font-bold text-gray-900 text-right py-2 item-row-total">${{ number_format($item->total, 2) }}</div>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 sticky top-8">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="tax_rate" id="tax_rate" value="{{ old('tax_rate', $invoice->tax_rate) }}" onchange="calculateTotals()"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="discount_amount" id="discount_amount" value="{{ old('discount_amount', $invoice->discount_amount) }}" onchange="calculateTotals()"
                                class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500">
                        </div>
                        ...
                        <div class="pt-4 border-t border-gray-50 space-y-3">
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <span class="text-gray-900 font-bold" id="subtotal-display">${{ number_format($invoice->subtotal, 2) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-500 font-medium">Tax</span>
                                <span class="text-gray-900 font-bold" id="tax-display">${{ number_format($invoice->tax_amount, 2) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-lg pt-2 border-t border-gray-100">
                                ...
                                <span class="text-blue-600 font-black" id="total-display">${{ number_format($invoice->total, 2) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8">
                        <textarea name="notes" id="notes" rows="3"
                            ...
                            placeholder="Additional notes...">{{ old('notes', $invoice->notes) }}</textarea>
                    </div>

                    <div class="mt-8">
                        <button type="submit" class="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all active:scale-[0.98]">
                            Update Invoice
                        </button>
                    </div>
                </div>
